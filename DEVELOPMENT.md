> Monorepo: desktop-приложение находится в pps/desktop. Команды из этого документа по-прежнему запускаются из корня репозитория через workspace-скрипты.
...
Lifecycle и delete dialog:

```bash
npm run test:run -- src/renderer/src/modules/study/lib/study-draft-lifecycle.test.ts src/renderer/src/modules/study/components/DeleteConfirmationDialog.test.tsx
```

Полный набор тестов безопасного удаления:

```bash
npm run test:run -- src/renderer/src/modules/study/lib/study-autosave-queue.test.ts src/renderer/src/modules/study/lib/study-draft-lifecycle.test.ts src/renderer/src/modules/study/lib/study-material-transition.test.ts src/renderer/src/modules/study/lib/study-tree.test.ts src/renderer/src/modules/study/hooks/use-study.test.tsx src/renderer/src/modules/study/components/DeleteConfirmationDialog.test.tsx
```
...
6. Диалог должен показать «Удаляем…».
7. Кнопки подтверждения и отмены должны стать недоступными.
8. После успеха материал и диалог должны исчезнуть.
9. После удаления не должно появиться нового save для удалённого material ID.
...
1. Запусти удаление.
2. Быстро нажми кнопку несколько раз.
3. Попробуй Shift+Enter.
4. В main process должен уйти только один delete IPC.
5. Пока запрос выполняется, закрытие диалога через Escape или кнопку отмены должно быть невозможно.

### Переход начинается до удаления

1. Измени материал A.
2. Сразу выбери материал B, чтобы начался flush.
3. До завершения сохранения открой удаление A и подтверди.
4. Переход не должен завершиться до commit или rollback delete.
5. После commit не должно быть повторного сохранения удалённого A.
6. После rollback восстановленный A должен пройти повторный flush.

### Переход начинается после удаления

1. Запусти удаление активного материала.
2. До ответа backend попробуй перейти к другому материалу или модулю.
3. Модальный диалог должен блокировать взаимодействие.
4. Программный переход обязан ждать deletion barrier.

### Ошибка удаления

Для проверки временно смоделируй ошибку `studyClient.deleteNode` или repository.
...
- материал остаётся в дереве;
- черновик остаётся в памяти;
- очередь выходит из deletion pause;
- последняя версия снова сохраняется;
- повторное удаление возможно.

### Ошибка rollback save

Ожидаемое поведение:

- delete возвращает ошибку;
- переход или shutdown не получают ложный успешный flush;
- material editor остаётся открытым;
- ошибка сохранения остаётся видимой пользователю.

### Закрытие приложения во время pending delete

1. Запусти удаление активного материала.
2. Сразу закрой приложение.
3. Renderer shutdown flush должен дождаться commit или rollback.
4. SQLite не должна закрываться до окончания tracked main-process delete.
5. При rollback save error закрытие должно быть отменено с сообщением.
...
- save из уничтоженного редактора не запускается;
- React state callback размонтированного редактора не вызывается;
- draft остаётся несохранённым до явного повторного mount.
...
2. ожидает завершения pending delete;
...
Tracked draft handle повторно проверяет deletion epoch после каждого асинхронного source flush. Поэтому shutdown не пропускает delete, начавшийся во время сохранения.
