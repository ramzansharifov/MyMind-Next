...
export function StudyInternalLinkPicker({
  ...
  const selectedOptionId =
    targets[state.selectedIndex]
      ? getInternalLinkOptionId(
          listboxId,
          state.selectedIndex
        )
      : undefined
      ...
      <Popover.Portal>
        ...
        >
          <header className="flex items-center gap-2 border-b border-[var(--app-border)] p-2">
            ...
            >
              <button
                ...
                className="flex size-9 shrink-0 items-center justify-center rounded-lg text-[var(--app-muted)] outline-none transition-colors hover:bg-white/[0.06] hover:text-[var(--app-text)] focus-visible:ring-2 focus-visible:ring-violet-500/35"
                onPointerDown={(event) => {
                  event.preventDefault()
                  event.stopPropagation()
                }}
              ...
              >
                ...
                targets.length === 0 ? (
                  ...
                  <div className="flex min-h-28 flex-col items-center justify-center px-6 text-center">
                    <Search
                      aria-hidden="true"
                      className="size-5 text-[var(--app-muted)]/60"
                    />
          ...
          <Popover.Arrow
            width={10}
            height={5}
            className="fill-[var(--app-surface-raised)]"
          />
        </Popover.Content>
      </Popover.Portal>
    </Popover.Root>
...
function getInternalLinkOptionId(
  listboxId: string,
  index: number
): string {
