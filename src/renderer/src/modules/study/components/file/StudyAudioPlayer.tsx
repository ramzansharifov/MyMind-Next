    ...
    <div className="mt-4 overflow-hidden rounded-xl border border-white/[0.07] bg-[#0a0b0f] shadow-inner shadow-black/20">
      ...
      <div className="flex items-center gap-3 p-3.5 max-[560px]:flex-wrap">
        <button
          ...
          className={cn(
            ...
            'outline-none transition-[background-color,transform,box-shadow]',
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex items-center justify-between font-mono text-[11px] tabular-nums text-[var(--app-muted)]">
            <span>
              {formatAudioTime(
                progressValue
              )}
      ...
      <div className="flex min-w-0 items-center justify-between gap-4 border-t border-white/[0.06] bg-white/[0.018] px-3.5 py-2.5">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          >
            ...
            <Slider.Thumb
              ...
              className="block size-3 rounded-full bg-white outline-none transition-transform hover:scale-125 focus-visible:ring-4 focus-visible:ring-violet-500/25"
            />
          </Slider.Root>

          <span className="w-8 shrink-0 text-right font-mono text-[10px] tabular-nums text-[var(--app-muted)] max-[480px]:hidden">
            {Math.round(
              effectiveVolume * 100
            )}
            %
        ...
        <button
          type="button"
          disabled={!isReady}
          aria-label={`Скорость воспроизведения ${formatPlaybackRate(
            playbackRate
          )}`}
