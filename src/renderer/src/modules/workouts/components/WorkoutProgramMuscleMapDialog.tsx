...
interface WorkoutProgramMuscleMapDialogProps {
  open: boolean
...
function ProgramBody({
  ...
  return (
    <BodyFigure
      ...
      activeGroup={null}
      glow
      idPrefix={`program-muscle-map-${view.toLowerCase()}-${id}`}
      width={340}
...
export function WorkoutProgramMuscleMapDialog({
  open,
  program,
  exercises,
  onOpenChange
  ...
  return (
    <AppDialog
      ...
      title={program ? `Карта мышц · ${program.name}` : 'Карта мышц программы'}
          ...
          <section className="overflow-hidden rounded-2xl border border-[var(--app-border)] bg-[var(--app-workspace)]">
            ...
            {analysis.rows.length === 0 ? (
              <div className="flex min-h-[440px] items-center justify-center px-6 text-center text-sm text-[var(--app-muted)]">
                В программе пока нет упражнений с указанными группами мышц.
              </div>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="rounded-lg bg-violet-500/10 px-2 py-1 text-[11px] font-semibold text-violet-300">
                      {row.count} {row.count === 1 ? 'упражнение' : 'упражнения'}
                    </span>
                  </div>
