    ...
    <main className="h-full overflow-y-auto bg-[var(--app-workspace)] px-8 py-7 max-[700px]:px-4 max-[700px]:py-5">
      <div className="mx-auto flex min-h-full w-full max-w-[980px] items-center justify-center">
        <section className="relative isolate w-full max-w-xl overflow-hidden rounded-[28px] border border-[var(--app-border)] bg-[var(--app-surface)] p-7 shadow-[var(--app-shadow-card)]">
          <div aria-hidden="true" className="pointer-events-none absolute -top-28 right-0 -z-10 size-72 rounded-full bg-violet-500/12 blur-3xl" />
          <div aria-hidden="true" className="pointer-events-none absolute -bottom-36 -left-20 -z-10 size-72 rounded-full bg-violet-900/10 blur-3xl" />
          ...
          <form className="mt-6 space-y-4" onSubmit={(event) => void submit(event)}>
            ...
            {!initialized && (
              <label className="block space-y-1.5">
                <span className="text-xs font-medium text-[var(--app-muted)]">Повторите мастер-пароль</span>
                <input
                  type="password"
                  value={confirmation}
            ...
            {!initialized && (
              <div className="rounded-xl border border-amber-400/15 bg-amber-500/[0.06] px-4 py-3 text-xs leading-5 text-amber-100/80">
                Если мастер-пароль будет утерян, расшифровать записи без него нельзя. Сохраните его в надёжном месте.
              </div>
            )}
...
export function PasswordsPage({ resourceId, onResourceHandled }: PasswordsPageProps): React.JSX.Element {
  ...
  async function saveGroup(input: CreatePasswordGroupInput | UpdatePasswordGroupInput): Promise<void> {
    setIsBusy(true)
    setError(null)
    try {
      const saved =
      ...
      <div className="mx-auto w-full max-w-[1400px]">
        <header className="relative isolate overflow-hidden rounded-[28px] border border-[var(--app-border)] bg-[var(--app-surface)] p-6 shadow-[var(--app-shadow-card)]">
          ...
          <div className="flex flex-wrap items-center justify-between gap-5">
            <div className="flex min-w-0 items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-semibold tracking-[-0.035em] text-[var(--app-text)]">Пароли</h1>
                <p className="mt-1 text-sm text-[var(--app-muted)]">Локальное зашифрованное хранилище логинов, паролей и секретных данных.</p>
              </div>
            </div>
        ...
        {error && (
          <div role="alert" className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-300">
            <span>{error}</span>
            <button type="button" aria-label="Закрыть ошибку" className="flex size-7 items-center justify-center rounded-lg hover:bg-red-500/10" onClick={() => setError(null)}>
              <X className="size-4" />
            </button>
          </div>
        )}
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[250px_minmax(0,1fr)]">
          <aside className="self-start rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-3 shadow-[var(--app-shadow-card)] lg:sticky lg:top-5">
            <div className="flex items-center justify-between px-2 py-1.5">
              ...
              <button type="button" aria-label="Создать группу" className="flex size-7 items-center justify-center rounded-lg text-[var(--app-muted)] hover:bg-[var(--app-control-hover)] hover:text-[var(--app-text)]" onClick={() => { setEditingGroup(null); setGroupDialogOpen(true) }}>
            ...
            <div className="mt-2 space-y-1">
              <button
                type="button"
                className={cn('flex h-10 w-full items-center gap-2 rounded-xl px-3 text-sm', groupFilter === 'all' ? 'bg-violet-500/12 font-semibold text-violet-200' : 'text-[var(--app-muted)] hover:bg-[var(--app-control-hover)] hover:text-[var(--app-text)]')}
              ...
              <button
                type="button"
                className={cn('flex h-10 w-full items-center gap-2 rounded-xl px-3 text-sm', groupFilter === 'ungrouped' ? 'bg-violet-500/12 font-semibold text-violet-200' : 'text-[var(--app-muted)] hover:bg-[var(--app-control-hover)] hover:text-[var(--app-text)]')}
              ...
              {groups.map((group) => {
                ...
                return (
                  <div key={group.id} className={cn('group flex items-center rounded-xl', selected && 'bg-[var(--app-control)]')}>
                    <button
                      type="button"
                      className={cn('flex h-11 min-w-0 flex-1 items-center gap-2.5 rounded-xl px-2.5 text-sm', selected ? 'font-semibold text-[var(--app-text)]' : 'text-[var(--app-muted)] hover:text-[var(--app-text)]')}
                    ...
                    <div className={cn('mr-1 flex shrink-0 items-center', selected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100')}>
                      <button type="button" aria-label={`Изменить группу «${group.name}»`} className="flex size-7 items-center justify-center rounded-lg text-[var(--app-muted)] hover:bg-[var(--app-control-hover)] hover:text-[var(--app-text)]" onClick={() => { setEditingGroup(group); setGroupDialogOpen(true) }}>
                        <Pencil className="size-3.5" />
                      </button>
                      <button type="button" aria-label={`Удалить группу «${group.name}»`} className="flex size-7 items-center justify-center rounded-lg text-[var(--app-muted)] hover:bg-red-500/10 hover:text-red-300" onClick={() => setDeleteGroupTarget(group)}>
            ...
            {view !== 'security' ? (
              <>
                <div className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-4 shadow-[var(--app-shadow-card)]">
                  <div className="flex flex-wrap gap-2">
                    <label className="flex h-11 min-w-[240px] flex-1 items-center gap-2 rounded-xl border border-[var(--app-border)] bg-[var(--app-workspace)] px-3.5 focus-within:border-violet-500/45 focus-within:ring-2 focus-within:ring-violet-500/10">
                      ...
                      <input value={query} type="search" aria-label="Поиск по паролям" placeholder="Название, логин, сайт или тег…" className="min-w-0 flex-1 bg-transparent text-sm text-[var(--app-text)] outline-none placeholder:text-[var(--app-muted)]/60" onChange={(event) => setQuery(event.target.value)} />
                      {query && <button type="button" aria-label="Очистить поиск" className="text-[var(--app-muted)] hover:text-[var(--app-text)]" onClick={() => setQuery('')}><X className="size-4" /></button>}
                    </label>
                ...
                {baseFilteredItems.length === 0 ? (
                  <div className="flex min-h-72 flex-col items-center justify-center rounded-2xl border border-dashed border-[var(--app-border)] bg-[var(--app-surface)] px-6 text-center">
                    <span className="flex size-14 items-center justify-center rounded-2xl border border-violet-500/15 bg-violet-500/10 text-violet-300"><KeyRound className="size-7" /></span>
                    <h2 className="mt-4 text-lg font-semibold text-[var(--app-text)]">{items.length === 0 ? 'Хранилище пока пустое' : 'Ничего не найдено'}</h2>
                    <p className="mt-2 max-w-md text-sm leading-6 text-[var(--app-muted)]">{items.length === 0 ? 'Добавьте первый аккаунт или отдельный пароль. Все секретные данные будут сохранены в зашифрованном виде.' : 'Измените группу, фильтры или строку поиска.'}</p>
                    {items.length === 0 && <button type="button" className="mt-5 inline-flex h-10 items-center gap-2 rounded-xl bg-violet-500 px-4 text-sm font-semibold text-white hover:bg-violet-400" onClick={() => { setEditingItem(null); setItemDialogOpen(true) }}><Plus className="size-4" /> Новая запись</button>}
                  </div>
                ) : (
                  <div className="overflow-hidden rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] shadow-[var(--app-shadow-card)]">
                    <div className="divide-y divide-[var(--app-border)]">
                      {baseFilteredItems.map((item) => {
                        ...
                        return (
                          <article key={item.id} className="group/item flex items-center gap-3 px-4 py-3.5 transition-colors hover:bg-[var(--app-control-hover)]">
                            <button type="button" className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-violet-400/15 bg-violet-500/10 text-violet-300" onClick={() => void openItem(item.id)}>
                              {item.type === 'login' ? <UserRound className="size-4" /> : <KeyRound className="size-4" />}
                            </button>
                            <button type="button" className="min-w-0 flex-1 text-left" onClick={() => void openItem(item.id)}>
                              <div className="flex flex-wrap items-center gap-2">
                                <span className="truncate text-sm font-semibold text-[var(--app-text)]">{item.title}</span>
                                {item.favorite && <Star className="size-3.5 fill-amber-300 text-amber-300" />}
                                <span className={cn('rounded-md border px-1.5 py-0.5 text-[10px] font-medium', passwordStrengthClassName(item.strength))}>{passwordStrengthLabel(item.strength)}</span>
                              </div>
                              <div className="mt-1 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 text-xs text-[var(--app-muted)]">
                                {item.username && <span className="max-w-[260px] truncate">{item.username}</span>}
                                {item.website && <span className="max-w-[260px] truncate">{item.website}</span>}
                                <span>{passwordTypeLabel(item.type)}</span>
                              </div>
                              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                                {group && (() => {
                                  const color = passwordGroupColorClasses[group.color]
                                  return <span className={cn('inline-flex h-6 items-center gap-1.5 rounded-lg border px-2 text-[11px] font-medium', color.soft, color.text, color.border)}><PasswordGroupIconGlyph icon={group.icon} className="size-3" />{group.name}</span>
                                })()}
                                {item.securityIssues.map((issue) => <span key={issue} className={cn('inline-flex h-6 items-center gap-1 rounded-lg border px-2 text-[11px] font-medium', passwordIssueClassName(issue))}><ShieldAlert className="size-3" />{passwordIssueLabel(issue)}</span>)}
                              </div>
                            </button>
                            <div className="flex shrink-0 items-center gap-1">
                              {item.username && <button type="button" aria-label={`Скопировать логин «${item.title}»`} className="flex size-8 items-center justify-center rounded-lg text-[var(--app-muted)] hover:bg-[var(--app-control)] hover:text-[var(--app-text)]" onClick={() => void copyField(item.id, 'username')}><UserRound className="size-3.5" /></button>}
                              <button type="button" aria-label={`Скопировать пароль «${item.title}»`} className="flex size-8 items-center justify-center rounded-lg text-violet-300 hover:bg-violet-500/10" onClick={() => void copyField(item.id, 'password')}><ClipboardCopy className="size-3.5" /></button>
                              <button type="button" aria-label={`Изменить «${item.title}»`} className="flex size-8 items-center justify-center rounded-lg text-[var(--app-muted)] opacity-0 group-hover/item:opacity-100 focus:opacity-100 hover:bg-[var(--app-control)] hover:text-[var(--app-text)]" onClick={() => void editItem(item.id)}><Pencil className="size-3.5" /></button>
                              <button type="button" aria-label={`Удалить «${item.title}»`} className="flex size-8 items-center justify-center rounded-lg text-[var(--app-muted)] opacity-0 group-hover/item:opacity-100 focus:opacity-100 hover:bg-red-500/10 hover:text-red-300" onClick={() => setDeleteItemTarget(item)}><Trash2 className="size-3.5" /></button>
                            </div>
                          </article>
                        )
                      })}
              ...
              <>
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  {[
                    { label: 'Всего', value: securityStats.total, icon: KeyRound, tone: 'text-violet-300' },
                    { label: 'Слабые', value: securityStats.weak, icon: ShieldAlert, tone: 'text-rose-300' },
                    { label: 'Повторяются', value: securityStats.reused, icon: ClipboardCopy, tone: 'text-orange-300' },
                    { label: 'Старше 180 дней', value: securityStats.old, icon: LockKeyhole, tone: 'text-amber-300' }
                  ].map((stat) => {
                    const Icon = stat.icon
                    return <div key={stat.label} className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-4 shadow-[var(--app-shadow-card)]"><div className="flex items-center justify-between gap-3"><span className="text-xs font-medium text-[var(--app-muted)]">{stat.label}</span><Icon className={cn('size-4', stat.tone)} /></div><div className="mt-2 text-2xl font-semibold text-[var(--app-text)]">{stat.value}</div></div>
                  })}
                </div>
                ...
                <div className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-4 shadow-[var(--app-shadow-card)]">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <h2 className="text-sm font-semibold text-[var(--app-text)]">Защита хранилища</h2>
                      <p className="mt-1 max-w-2xl text-xs leading-5 text-[var(--app-muted)]">Секретные поля и названия групп хранятся зашифрованными. Модуль автоматически блокируется через 5 минут бездействия и при уходе со страницы. Скопированный секрет очищается из буфера обмена через 30 секунд, если вы не заменили его другим содержимым.</p>
                    </div>
                    <button type="button" className="inline-flex h-10 items-center gap-2 rounded-xl border border-[var(--app-border)] bg-[var(--app-workspace)] px-4 text-sm font-medium text-[var(--app-muted)] hover:bg-[var(--app-control-hover)] hover:text-[var(--app-text)]" onClick={() => setChangeMasterOpen(true)}><ShieldCheck className="size-4" /> Сменить мастер-пароль</button>
                  </div>
                </div>

                <div className="overflow-hidden rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] shadow-[var(--app-shadow-card)]">
                  <div className="flex items-center gap-2 border-b border-[var(--app-border)] px-4 py-3"><ShieldAlert className="size-4 text-violet-300" /><h2 className="text-sm font-semibold text-[var(--app-text)]">Требуют внимания</h2><span className="ml-auto rounded-lg bg-[var(--app-control)] px-2 py-0.5 text-xs text-[var(--app-muted)]">{groupScopedItems.filter((item) => item.securityIssues.length > 0).length}</span></div>
                  {groupScopedItems.filter((item) => item.securityIssues.length > 0).length === 0 ? (
                    <div className="flex min-h-52 flex-col items-center justify-center px-6 text-center"><ShieldCheck className="size-8 text-emerald-300" /><h3 className="mt-3 text-sm font-semibold text-[var(--app-text)]">Явных проблем не найдено</h3><p className="mt-1 text-xs text-[var(--app-muted)]">В выбранной группе нет слабых, повторяющихся или давно не обновлявшихся паролей.</p></div>
                  ) : (
                    <div className="divide-y divide-[var(--app-border)]">
                      {groupScopedItems.filter((item) => item.securityIssues.length > 0).map((item) => (
                        <button key={item.id} type="button" className="flex w-full items-center gap-3 px-4 py-3.5 text-left hover:bg-[var(--app-control-hover)]" onClick={() => void openItem(item.id)}>
                          <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-rose-500/10 text-rose-300"><ShieldAlert className="size-4" /></span>
                          <span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-[var(--app-text)]">{item.title}</span>{item.username && <span className="mt-0.5 block truncate text-xs text-[var(--app-muted)]">{item.username}</span>}</span>
                          <span className="flex flex-wrap justify-end gap-1.5">{item.securityIssues.map((issue) => <span key={issue} className={cn('rounded-lg border px-2 py-1 text-[11px] font-medium', passwordIssueClassName(issue))}>{passwordIssueLabel(issue)}</span>)}</span>
                        </button>
                      ))}
                    </div>
                  )}
