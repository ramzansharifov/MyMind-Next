...
function FilterSelect({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <AppSelect ariaLabel={label} value={value} options={selectOptions} onValueChange={onChange} />
    </div>
  )
}
...
export function MusicPage({ resourceId, onResourceHandled }: MusicPageProps): React.JSX.Element {
  ...
  return (
    <StandardModulePage>
      <ModuleHeader icon={Disc3} title={headerTitle} className="mb-5" actions={headerActions}>
        {view.kind === 'library' && (
          <>
            <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 max-[1120px]:grid-cols-1">
              ...
              <div className="flex min-w-0 items-stretch gap-2 max-[760px]:flex-col">
                <div
                  ...
                  className="flex min-h-12 min-w-0 items-center gap-1 overflow-x-auto rounded-2xl border border-[var(--app-border)] bg-[var(--app-workspace)] p-1.5"
                >
                  {tabs.map((item) => (
                    <button
                      key={item.id}
                ...
                <Popover.Root>
                  <Popover.Trigger asChild>
                    ...
                    >
                      ...
                      {activeFilterCount > 0 && (
                        <span className="rounded-md bg-violet-400/15 px-1.5 text-[11px]">
                          {activeFilterCount}
                        </span>
                      )}
                    </button>
                  ...
                  <Popover.Portal>
                    <Popover.Content
                      ...
                      sideOffset={8}
                      collisionPadding={12}
                      className="z-[70] w-[min(32rem,calc(100vw-2rem))] rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-raised)] p-4 shadow-2xl outline-none"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-[var(--app-muted)]">
                            Тип, исполнитель, жанр, год и оценка.
                          </p>
                        </div>
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-2">
                        ...
                        <FilterSelect
                          ...
                          onChange={(value) => setAdvancedFilter('artist', value)}
                        >
            ...
            {activeFilterCount > 0 && (
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                {filters.type !== 'all' && (
                  <ActiveFilterChip
                    label={`Тип: ${musicTypeLabel(filters.type)}`}
                ...
                {filters.artist !== 'all' && (
                  <ActiveFilterChip
                    ...
                    onRemove={() => clearAdvancedFilter('artist')}
                  />
                )}
                {filters.year !== 'all' && (
                  <ActiveFilterChip
                ...
                {filters.minRating !== 'all' && (
                  <ActiveFilterChip
                    label={`Оценка: от ${filters.minRating}`}
                    onRemove={() => clearAdvancedFilter('minRating')}
                  />
                )}
                <button
                  type="button"
                  className="h-8 rounded-lg px-2.5 text-xs font-medium text-[var(--app-muted)] hover:bg-[var(--app-control-hover)]"
                  onClick={() => setFilters({ ...EMPTY_FILTERS })}
                >
                  Сбросить все
                </button>
      ...
      {view.kind === 'form' ? (
        ...
        <div className="flex min-h-80 flex-col items-center justify-center rounded-[24px] border border-dashed border-[var(--app-border)] bg-[var(--app-surface)] px-6 text-center">
          ...
          <h2 className="mt-4 text-lg font-semibold text-[var(--app-text)]">
            {items.length === 0 ? 'Музыкальная библиотека пока пустая' : 'Ничего не найдено'}
          </h2>
          <p className="mt-2 max-w-md text-sm leading-6 text-[var(--app-muted)]">
            {items.length === 0
              ? 'Добавьте первый трек, альбом, EP или сингл.'
              : 'Попробуйте изменить поиск, раздел или активные фильтры.'}
          </p>
          {items.length === 0 && (
            <button
              type="button"
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(190px,1fr))] gap-x-4 gap-y-7 sm:grid-cols-[repeat(auto-fill,minmax(205px,1fr))]">
          {visibleItems.map((item) => {
            const artist =
              item.artists.length > 0 ? item.artists.join(', ') : 'Исполнитель не указан'
              ...
              <article key={item.id} className="music-card group min-w-0">
                <div className="music-card-body overflow-hidden rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] shadow-[var(--app-shadow-card)]">
                  <div className="relative aspect-square overflow-hidden bg-[var(--app-workspace)]">
                    <button
                      type="button"
                      aria-label={`Открыть музыку «${item.title}»`}
                    ...
                    <button
                      ...
                      className={
                        item.favorite
                          ? 'absolute top-2.5 right-2.5 z-10 flex size-9 items-center justify-center rounded-xl border border-rose-300/25 bg-black/50 text-rose-300 backdrop-blur-md hover:bg-black/65'
                  ...
                  <button
                    type="button"
                    className="flex min-h-20 w-full min-w-0 flex-col items-start justify-center border-t border-[var(--app-border)] bg-[var(--app-surface)] px-3.5 py-3 text-left outline-none hover:bg-[var(--app-control-hover)]"
                    onClick={() => setView({ kind: 'detail', itemId: item.id })}
                  >
                    <span className="block w-full truncate text-sm font-semibold text-[var(--app-text)] group-hover:text-violet-200">
                      {item.title}
                    </span>
                    ...
                    <span className="mt-1 flex items-center gap-2 text-[10px] text-[var(--app-muted)]/80">
                      <span>{musicTypeLabel(item.type)}</span>
                      {duration && (
                        <>
                          <span>•</span>
                          <span>{duration}</span>
