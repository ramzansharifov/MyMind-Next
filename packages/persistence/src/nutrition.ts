...
export interface NutritionRepository {
  listOverview(input: NutritionOverviewInput): NutritionOverview
  createFood(input: CreateNutritionFoodInput): NutritionFoodRecord
  createFoods(input: CreateNutritionFoodsInput): CreateNutritionFoodsResult
  updateFood(input: UpdateNutritionFoodInput): NutritionFoodRecord
  deleteFood(input: DeleteNutritionFoodInput): boolean
  createRecipe(input: CreateNutritionRecipeInput): NutritionRecipeRecord
  updateRecipe(input: UpdateNutritionRecipeInput): NutritionRecipeRecord
  deleteRecipe(input: DeleteNutritionRecipeInput): boolean
  createLogEntry(input: CreateNutritionLogEntryInput): NutritionLogEntryRecord
  importMeals(input: ImportNutritionMealsInput): ImportNutritionMealsResult
  updateLogEntry(input: UpdateNutritionLogEntryInput): NutritionLogEntryRecord
  deleteLogEntry(input: DeleteNutritionLogEntryInput): boolean
  setWater(input: SetNutritionWaterInput): NutritionDaySummary
  setTargets(input: SetNutritionTargetsInput): NutritionTargetRecord
  getReport(input: NutritionReportInput): NutritionReport
}

export function createNutritionRepository(runtime: RepositoryRuntime): NutritionRepository {
  const getDatabase = runtime.database
  const createId = runtime.createId
  ...
  function requireFood(id: string): NutritionFoodRecord {
    const row = getDatabase().prepare(`${FOOD_SELECT} WHERE id = ?`).get(id) as FoodRow | undefined
    if (!row) throw new Error('Продукт не найден')
    return mapFood(row)
  }

  function requireRecipeRow(id: string): RecipeRow {
    const row = getDatabase().prepare(`${RECIPE_SELECT} WHERE id = ?`).get(id) as
      RecipeRow | undefined
  ...
  function requireLog(id: string): NutritionLogEntryRecord {
    const row = getDatabase().prepare(`${LOG_SELECT} WHERE id = ?`).get(id) as LogRow | undefined
  ...
  function listNutritionOverview(date: string): NutritionOverview {
    const foods = (
      getDatabase().prepare(`${FOOD_SELECT} ORDER BY name COLLATE NOCASE ASC`).all() as FoodRow[]
    ).map(mapFood)
    const recipes = (
      getDatabase()
  ...
  function createNutritionFood(input: CreateNutritionFoodInput): NutritionFoodRecord {
    return insertNutritionFood(input)
  }

  function createNutritionFoods(input: CreateNutritionFoodsInput): CreateNutritionFoodsResult {
    ...
    const transaction = getDatabase().transaction((foods: CreateNutritionFoodInput[]) =>
  ...
  function updateNutritionFood(input: UpdateNutritionFoodInput): NutritionFoodRecord {
  ...
  function deleteNutritionFood(input: DeleteNutritionFoodInput): boolean {
    ...
    return (
      getDatabase().prepare('DELETE FROM nutrition_foods WHERE id = ?').run(input.id).changes > 0
    )
  }
  ...
  function replaceRecipeIngredients(
    ...
    ingredients.forEach((ingredient, position) => {
      ...
      insert.run(createId(), recipeId, ingredient.foodId, toMilli(ingredient.amount), position)
    })
  }

  function createNutritionRecipe(input: CreateNutritionRecipeInput): NutritionRecipeRecord {
    const db = getDatabase()
    const id = createId()
    const now = currentTime()
    db.transaction(() => {
  ...
  function updateNutritionRecipe(input: UpdateNutritionRecipeInput): NutritionRecipeRecord {
    ...
    db.transaction(() => {
      db.prepare(
        `UPDATE nutrition_recipes SET name = ?, description = ?, servings_milli = ?,
         updated_at = ? WHERE id = ?`
      ).run(input.name.trim(), input.description, toMilli(input.servings), currentTime(), input.id)
      replaceRecipeIngredients(input.id, input.ingredients)
    })()
    return requireRecipe(input.id)
  }

  function deleteNutritionRecipe(input: DeleteNutritionRecipeInput): boolean {
    return (
      getDatabase().prepare('DELETE FROM nutrition_recipes WHERE id = ?').run(input.id).changes > 0
    )
  }
  ...
  function createNutritionLogEntry(input: CreateNutritionLogEntryInput): NutritionLogEntryRecord {
    const id = createId()
    writeLogEntry(id, input, false)
    return requireLog(id)
  }

  function updateNutritionLogEntry(input: UpdateNutritionLogEntryInput): NutritionLogEntryRecord {
    requireLog(input.id)
    writeLogEntry(input.id, input, true)
    return requireLog(input.id)
  }

  function deleteNutritionLogEntry(input: DeleteNutritionLogEntryInput): boolean {
    return (
      getDatabase().prepare('DELETE FROM nutrition_log_entries WHERE id = ?').run(input.id)
        .changes > 0
    )
  }

  function setNutritionWater(input: SetNutritionWaterInput): NutritionDaySummary {
    getDatabase()
      .prepare(
      ...
      .run(input.date, input.waterMl, currentTime())
    return makeDaySummary(input.date)
  }

  function setNutritionTargets(input: SetNutritionTargetsInput): NutritionTargetRecord {
    const db = getDatabase()
    const id = createId()
    const now = currentTime()
  ...
  function getNutritionReport(input: NutritionReportInput): NutritionReport {
    ...
    const filteredEntries = (
      getDatabase()
        .prepare(
          `${LOG_SELECT} WHERE ${conditions.join(' AND ')} ORDER BY date ASC, created_at ASC`
        )
  ...
  function importNutritionMeals(input: ImportNutritionMealsInput): ImportNutritionMealsResult {
    const transaction = getDatabase().transaction((payload: ImportNutritionMealsInput) => {
  ...
  return {
    listOverview: (input) => listNutritionOverview(input.date),
    createFood: createNutritionFood,
    createFoods: createNutritionFoods,
    updateFood: updateNutritionFood,
    deleteFood: deleteNutritionFood,
    createRecipe: createNutritionRecipe,
    updateRecipe: updateNutritionRecipe,
    deleteRecipe: deleteNutritionRecipe,
    createLogEntry: createNutritionLogEntry,
    importMeals: importNutritionMeals,
    updateLogEntry: updateNutritionLogEntry,
    deleteLogEntry: deleteNutritionLogEntry,
    setWater: setNutritionWater,
    setTargets: setNutritionTargets,
    getReport: getNutritionReport
  }
}
