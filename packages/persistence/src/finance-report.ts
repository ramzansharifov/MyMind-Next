...
} from '@mymind/contracts/finance-report-analytics'
...
import { assertSafeMinor } from '@mymind/core/finance-money'
import { previousComparablePeriod } from '@mymind/core/finance-periods'

export interface FinanceReportAnalyticsDependencies {
  database(): SqlDatabasePort
  getSettings(): FinanceSettings
  listExchangeRates(): FinanceExchangeRate[]
  listLimits(at?: number): FinanceLimitStatus[]
}
...
function loadRateBook(dependencies: FinanceReportAnalyticsDependencies): {
  ...
  return {
    ...
    rateBook: createFinanceRateBook(settings.baseCurrencyCode, dependencies.listExchangeRates())
  }
}

function buildAggregateRows(
  dependencies: FinanceReportAnalyticsDependencies,
...
function attachBalanceTimeline(
  ...
  const accounts = loadBalanceAccounts(dependencies, accountIds)
  const entries = loadBalanceEntries(dependencies, period.to, accountIds)
...
function buildPeriodData(
  ...
  const raw = buildAggregateRows(dependencies, filters)
...
export function getFinanceReportAnalytics(
  dependencies: FinanceReportAnalyticsDependencies,
  filters: FinanceReportFilters
): FinanceReportAnalytics {
  const { targetDefaultCurrency, rateBook } = loadRateBook(dependencies)
  ...
  const current = buildPeriodData(dependencies, filters, targetCurrency, rateBook)
  ...
  return {
    ...
    transferFlows,
    limits: relevantLimits(dependencies, filters)
  }
}
