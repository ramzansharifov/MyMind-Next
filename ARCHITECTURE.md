...
Если удаляется папка, renderer проверяет всё поддерево. Suspension создаётся, когда активный material ID находится внутри удаляемой папки.

Зарегистрированный `StudyDraftHandle` оборачивает suspension и хранит Promise результата удаления.

Пока delete не завершён, handle сообщает о наличии незавершённой работы, даже когда сам документ был чистым.

Поэтому следующие операции ждут результата delete:
...
- узел и его потомки удаляются из renderer state;
...
- если редактор всё ещё смонтирован, последняя версия черновика сохраняется;
- ожидающие переходы и shutdown продолжаются только после rollback;
- сообщение ошибки отображается в интерфейсе.

Если редактор уже размонтирован, rollback не может повторно активировать очередь и не отправляет фоновое сохранение из уничтоженного компонента.

Ошибка rollback-сохранения передаётся ожидающему переходу или shutdown и не маскируется как успешный flush.

Изменения, сделанные пользователем во время pending delete, остаются в очереди и сохраняются после rollback, пока редактор остаётся смонтированным.

Активный IPC save нельзя физически отменить. Main-process material coordinator гарантирует, что save и delete одного материала выполняются последовательно.
...
4. pending delete, если он есть, сначала завершается commit или rollback;
