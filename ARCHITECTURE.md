...
2. переводит autosave queue в pause;
...
- queue возобновляется;
- последняя версия черновика сохраняется;
- активный материал остаётся открытым;
- сообщение ошибки отображается в интерфейсе.

Изменения, сделанные пользователем во время pending delete, остаются в очереди и сохраняются после rollback.
