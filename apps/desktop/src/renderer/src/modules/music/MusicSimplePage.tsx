...
export function MusicPage({ resourceId, onResourceHandled }: MusicPageProps): React.JSX.Element {
  ...
  const currentOverview = normalizeMusicOverview(overview)
  ...
  function openTrackEditor(itemId: string): void {
    const item = currentOverview.items.find((entry) => entry.id === itemId)
    if (!item) return
    setTrackDialogItem(item)
    setTrackDialogOpen(true)
  }
  ...
  function openTrackJson(item: MusicItemRecord): void {
    setJsonView({
      ...
      value: item,
      note: 'Это полный MusicItemRecord. При повторной вставке тот же id обновит этот трек на месте; createdAt и updatedAt из JSON не перезаписываются.'
    })
  }
  ...
  function openLibraryJson(): void {
    setJsonView({
      ...
      note: 'Это полный MusicOverview: items содержит треки, playlists — плейлисты и их trackIds. Весь этот JSON можно вставить обратно: существующие id обновятся, новые записи добавятся, связи плейлистов применятся.'
    })
  }
  ...
  async function importMusic(input: UpsertMusicLibraryInput): Promise<void> {
    ...
    setError(null)
    try {
      await musicClient.upsertLibrary(input)
      await refreshOverview()
    } catch (reason) {
      setError(errorMessage(reason))
      throw reason
